...
class CallbackHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        print(f"   Code extraído: {callback_data['code'][:30] if callback_data['code'] else 'None'}...")
        print(f"   State extraído: {callback_data['state'][:30] if callback_data['state'] else 'None'}...")
...
def test_provider(provider: str) -> bool:
    ...
    try:
        ...
        other_fields = {
            ...
            if k not in user_fields and v is not None
        }
        if other_fields:
            print(f"\n   Campos adicionais:")
        ...
        print(f"\n📊 Resumo:")
        user_id = (
            user_dict.get('email') or
            user_dict.get('preferred_username') or
            user_dict.get('sub', 'N/A')
        )
