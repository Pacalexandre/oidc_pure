.PHONY: post-create test-tools init sync lint format type-check gitignore freeze dev-tools ensure-ipykernel venv
...
sync:
	...
	@uv sync --extra dev --extra test > /tmp/uv-sync.log 2>&1
